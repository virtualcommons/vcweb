...
def convert_to_empty_string(model_class, *attributes):
    for attribute in attributes:
        none_dict = { attribute: None }
        empty_string_dict = { attribute: '' }
        model_class.objects.filter(**none_dict).update(**empty_string_dict)
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        GroupClusterDataValue = orm['core.GroupClusterDataValue']
        convert_to_empty_string(GroupClusterDataValue,'string_value')

        GroupRoundDataValue = orm['core.GroupRoundDataValue']
        convert_to_empty_string(GroupRoundDataValue,'string_value')

        Institution = orm['core.Institution']
        convert_to_empty_string(Institution,'description')

        Experimenter = orm['core.Experimenter']
        convert_to_empty_string(Experimenter, 'authentication_token')

        Parameter = orm['core.Parameter']
        convert_to_empty_string(Parameter, 'display_name', 'description', 'class_name', 'default_value_string', 'enum_choices')

        ParticipantExperimentRelationship = orm['core.ParticipantExperimentRelationship']
        convert_to_empty_string(ParticipantExperimentRelationship, 'current_location', 'additional_data')

        Group = orm['core.Group']
        convert_to_empty_string(Group, 'session_id')

        convert_to_empty_string(orm['core.RoundConfiguration'], 'debriefing', 'template_filename', 'session_id', 'template_id', 'instructions')

        convert_to_empty_string(orm['core.ExperimentSession'], 'invitation_text')

        convert_to_empty_string(orm['core.RoundParameterValue'], 'string_value')

        convert_to_empty_string(orm['core.ParticipantRoundDataValue'], 'string_value')

        convert_to_empty_string(orm['core.Participant'], 'authentication_token')

        convert_to_empty_string(orm['core.ExperimentMetadata'], 'description')

        convert_to_empty_string(orm['core.Experiment'], 'tick_duration', 'duration')

        convert_to_empty_string(orm['core.ExperimentParameterValue'], 'string_value')

        convert_to_empty_string(orm['core.GroupCluster'], 'session_id', 'name')

        convert_to_empty_string(orm['core.ExperimentConfiguration'], 'invitation_text', 'treatment_id', 'invitation_subject')

        convert_to_empty_string(orm['core.RoundData'], 'experimenter_notes')
