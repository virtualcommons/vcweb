...
# generate a GroupRoundDataValue for this location (this can be shortened)
        # round_data.group_data_value_set.create(group=group, parameter=pollution_parameter, value=location)
            grdv = group.data_value_set.create(round_data=round_data, parameter=pollution_parameter, value=location)
...
def pollution_url_maker(pk, pollution_symbol):
    pollution_url = "<a id='%d' class='pollution' href='#'>%s</a>" % (pk, pollution_symbol)
    ...
    #    ajax call here
    #    });
    # will probably need to use jQuery .delegate or .live to handle this properly
