...
# prune old connections
    ...
    def all_participants(self, connection, experiment):
        if connection in self.connection_to_experimenter:
            (experimenter_pk, experiment_pk) = self.connection_to_experimenter[
                connection]
            if experiment.pk == experiment_pk:
                ...
                logger.warning(
                    "Experimenter %s tried to refresh experiment %s", experimenter_pk, experiment)
        else:
            logger.warning(
                "No experimenter available for connection %s", connection)
    '''
    ...
    def send_refresh(self, connection, experiment, experimenter_id=None):
        for (participant_group_pk, connection) in self.all_participants(connection, experiment):
            logger.debug(
                "sending refresh to %s, %s", participant_group_pk, connection)
    ...
    def send_to_experimenter(self, experimenter_tuple, message):
        ...
        logger.debug(
            "sending %s to experimenter %s", message, experimenter_tuple)
        if experimenter_tuple in self.experimenter_to_connection:
            ...
            logger.debug(
                "all experimenters: %s", self.experimenter_to_connection)
...
class ExperimenterHandler(tornadio2.SocketConnection):
    ...
    def on_open(self, *args, **kwargs):
        try:
            ...
            logger.warning(
                "Tried to establish connection but there isn't any experimenter with id %s", experimenter_id)
    ...
    def on_message(self, message):
        ...
        if event.message_type == 'connect':
            ...
            experiment = Experiment.objects.get(pk=experiment_id)
            url = event.message
            notified_participants = connection_manager.send_goto(
                self, experiment, url)
            self.send(info_json("Sent goto:%s to all participants" % url))
            logger.debug(
                "sending all connected participants %s to %s", notified_participants, url)
...
class ParticipantHandler(tornadio2.SocketConnection):
    ...
    def on_open(self, *args, **kwargs):
        ...
        try:
            participant_experiment_relationship = ParticipantExperimentRelationship.objects.select_related(
                depth=1).get(pk=relationship_id)
            connection_manager.add_participant(
                extra, self, participant_experiment_relationship)
            ...
            logger.debug(
                "no participant experiment relationship with id %s (%s)", relationship_id, e)
    ...
    def on_message(self, message, *args, **kwargs):
        ...
        if 'connect' in message_type:
            ...
            (participant_pk, experiment_pk) = connection_manager.get_participant_experiment_tuple(
                self)
            experiment = Experiment.objects.get(pk=experiment_pk)
            logger.debug(
                "processing participant submission for participant %s and experiment %s", participant_pk, experiment)
            ...
            if experiment.is_data_round_in_progress:
                ...
                participant_group_relationship = ParticipantGroupRelationship.objects.get(
                    pk=pgr_pk)
                prdv = experiment.current_round_data.participant_data_value_set.get(
                    participant_group_relationship__pk=pgr_pk)
                ...
                logger.debug(
                    "No data round in progress, received late submit event: %s", event)

        elif message_type == 'chat':
            try:
                participant_group_relationship = connection_manager.get_participant_group_relationship(
                    self)
                ...
                connection_manager.send_to_group(
                    participant_group_relationship.group, chat_json)
            except:
                logger.warning(
                    "Couldn't find a participant group relationship using connection %s with connection manager %s", self, self.connection_manager)
...
def main(argv=None):
    ...
    application = tornado.web.Application(
        list(chain.from_iterable(
            [participantRouter.urls, experimenterRouter.urls, ])),
